...
func NewSimApp(
	...
	codecProxy, interfaceReg := fibochaincodec.MakeCodecSuit(ModuleBasics)
	...
	app.AccountKeeper = auth.NewAccountKeeper(
		codecProxy.GetCdc(), keys[auth.StoreKey], keys[mpt.StoreKey], app.subspaces[auth.ModuleName], fibochain.ProtoAccount,
	)
...
func updateGPOHandler(gpo *gasprice.Oracle) sdk.UpdateGPOHandler {
	return func(dynamicGpInfos []sdk.DynamicGasInfo) {
		if appconfig.GetFecConfig().GetDynamicGpMode() != fibochain.MinimalGpMode {
...
func (app *SimApp) SetOption(req abci.RequestSetOption) (res abci.ResponseSetOption) {
	if req.Key == "CheckChainID" {
		if err := fibochain.IsValidateChainIdWithGenesisHeight(req.Value); err != nil {
			app.Logger().Error(err.Error())
			panic(err)
		}
		err := fibochain.SetChainId(req.Value)
		if err != nil {
			app.Logger().Error(err.Error())
			panic(err)
		}
