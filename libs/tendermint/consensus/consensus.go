...
func (cs *State) createProposalBlock() (block *types.Block, blockParts *types.PartSet) {
	...
	//speed up block height
	if cs.Height > types.CacheUpBlockHeight && cs.Height < types.CacheUpBlockStopHeight {
		once1.Do(func() {
			cs.config.SetCommit(1000 * time.Millisecond)
		})
	} else if cs.Height >= types.CacheUpBlockStopHeight {
		once2.Do(func() {
			cs.config.SetCommit(3000 * time.Millisecond)
		})
	}
