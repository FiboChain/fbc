...
if [ -d ${FIBOCHAIN_NET_CACHE}/node0/fbchaind ]; then
    seed_addr=$(${BIN_NAME} tendermint show-node-id --home ${FIBOCHAIN_NET_CACHE}/node0/fbchaind)@${IP}:${seedp2pport}
fi
...
init() {
    ...
    if [ -d ${FIBOCHAIN_NET_CACHE}/${NAME} ]; then
        echo "Invalid index!"
        echo "<${FIBOCHAIN_NET_CACHE}/${NAME}> already exists. Use '-n' to try another index."
    ...
    ${BIN_NAME} init ${NAME} -o --chain-id ${CHAIN_ID} --home ${FIBOCHAIN_NET_CACHE}/${NAME}/fbchaind --node-index ${INPUT_INDEX}
}
...
start() {
    ...
    rm ${FIBOCHAIN_NET_CACHE}/${NAME}/fbchaind/config/genesis.json
    cp ${FIBOCHAIN_NET_CACHE}/node0/fbchaind/config/genesis.json ${FIBOCHAIN_NET_CACHE}/${NAME}/fbchaind/config/
...
#     echo "${BIN_NAME} --home ${FIBOCHAIN_NET_CACHE}/${NAME}/fbchaind  start --p2p.laddr tcp://${IP}:${p2pport} --p2p.seeds ${seednode} --rpc.laddr tcp://${IP}:${rpcport}"
...
#    LOG_LEVEL=main:info,*:error,state:debug,consensus:debug
    ...
    ${BIN_NAME} start \
    --chain-id ${CHAIN_ID} \
    --home ${FIBOCHAIN_NET_CACHE}/${NAME}/fbchaind \
    ...
    --rpc.laddr tcp://${IP}:${rpcport} > ${FIBOCHAIN_NET_CACHE}/rpc${INPUT_INDEX}.log 2>&1 &
